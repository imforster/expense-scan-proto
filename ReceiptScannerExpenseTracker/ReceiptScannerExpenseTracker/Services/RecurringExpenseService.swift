...
enum RecurringExpenseError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .templateNotFound: return "Recurring expense template not found"
        case .invalidTemplate: return "Invalid recurring expense template"
        case .synchronizationFailed(let message): return "Template synchronization failed: \(message)"
...
class RecurringExpenseService {
    ...
    func createRecurringExpense(
        amount: NSDecimalNumber, currencyCode: String, merchant: String, notes: String? = nil,
        paymentMethod: String? = nil, category: ReceiptScannerExpenseTracker.Category? = nil, tags: [ReceiptScannerExpenseTracker.Tag] = [], patternType: RecurringFrequency,
        interval: Int32 = 1, dayOfMonth: Int32? = nil, dayOfWeek: Int32? = nil, startDate: Date = Date()
    ) -> ReceiptScannerExpenseTracker.RecurringExpense {
        let recurringExpense = ReceiptScannerExpenseTracker.RecurringExpense(context: context)
        ...
        // Add tags
        for tag in tags { recurringExpense.addToTags(tag) }
        // Create pattern
        ...
        pattern.nextDueDate = calculateInitialDueDate(
            from: startDate, pattern: patternType, interval: interval, dayOfMonth: dayOfMonth)
        recurringExpense.pattern = pattern
        return recurringExpense
    }
    /// Convert an existing expense to a recurring template
    func convertExpenseToRecurring(
        expense: ReceiptScannerExpenseTracker.Expense, patternType: RecurringFrequency, interval: Int32 = 1, dayOfMonth: Int32? = nil,
        dayOfWeek: Int32? = nil
    ) -> ReceiptScannerExpenseTracker.RecurringExpense {
        return createRecurringExpense(
            amount: expense.amount, currencyCode: expense.currencyCode, merchant: expense.merchant,
            notes: expense.notes, paymentMethod: expense.paymentMethod, category: expense.category,
            tags: expense.safeTags, patternType: patternType, interval: interval, dayOfMonth: dayOfMonth,
            dayOfWeek: dayOfWeek, startDate: expense.date)
    }
    // MARK: - Querying Recurring Expenses

    /// Get all active recurring expenses
    func getActiveRecurringExpenses() -> [ReceiptScannerExpenseTracker.RecurringExpense] {
        let request: NSFetchRequest<ReceiptScannerExpenseTracker.RecurringExpense> = ReceiptScannerExpenseTracker.RecurringExpense.fetchRequest()
        request.predicate = NSPredicate(format: "isActive == YES")
        request.sortDescriptors = [NSSortDescriptor(keyPath: \ReceiptScannerExpenseTracker.RecurringExpense.merchant, ascending: true)]
    ...
    func getDueRecurringExpenses() -> [ReceiptScannerExpenseTracker.RecurringExpense] {
        let request: NSFetchRequest<ReceiptScannerExpenseTracker.RecurringExpense> = ReceiptScannerExpenseTracker.RecurringExpense.fetchRequest()
        request.predicate = NSPredicate(format: "isActive == YES AND pattern.nextDueDate <= %@", Date() as CVarArg)
        request.sortDescriptors = [NSSortDescriptor(keyPath: \ReceiptScannerExpenseTracker.RecurringExpense.pattern!.nextDueDate, ascending: true)]
        do { return try context.fetch(request) } catch {
    ...
    func getRecurringExpenses(for category: ReceiptScannerExpenseTracker.Category) -> [ReceiptScannerExpenseTracker.RecurringExpense] {
        let request: NSFetchRequest<ReceiptScannerExpenseTracker.RecurringExpense> = ReceiptScannerExpenseTracker.RecurringExpense.fetchRequest()
    ...
    func generateDueExpenses() -> [ReceiptScannerExpenseTracker.Expense] {
        let dueRecurringExpenses = getDueRecurringExpenses()
        var generatedExpenses: [ReceiptScannerExpenseTracker.Expense] = []
        for recurringExpense in dueRecurringExpenses {
            if let expense = generateExpense(from: recurringExpense) { generatedExpenses.append(expense) }
        }
        return generatedExpenses
    }
    /// Generate a single expense from a recurring template
    func generateExpense(from recurringExpense: ReceiptScannerExpenseTracker.RecurringExpense) -> ReceiptScannerExpenseTracker.Expense? {
        guard recurringExpense.isActive, let pattern = recurringExpense.pattern, pattern.nextDueDate <= Date() else {
            return nil
        }
    ...
    func deactivateRecurringExpense(_ recurringExpense: ReceiptScannerExpenseTracker.RecurringExpense) { recurringExpense.isActive = false }
    /// Reactivate a recurring expense
    func reactivateRecurringExpense(_ recurringExpense: ReceiptScannerExpenseTracker.RecurringExpense) {
        recurringExpense.isActive = true
        // Update next due date
        if let pattern = recurringExpense.pattern { pattern.nextDueDate = recurringExpense.calculateNextDueDate() }
    }
    /// Delete a recurring expense and optionally its generated expenses
    func deleteRecurringExpense(_ recurringExpense: ReceiptScannerExpenseTracker.RecurringExpense, deleteGeneratedExpenses: Bool = false) {
        ...
        if deleteGeneratedExpenses {
            ...
            for expense in generatedExpenses {
                if !expense.isDeleted && expense.managedObjectContext != nil { context.delete(expense) }
            }
        } else {
            // Clear the relationship but keep the expenses
            let generatedExpenses = Array(recurringExpense.safeGeneratedExpenses)
            for expense in generatedExpenses {
                if !expense.isDeleted && expense.managedObjectContext != nil { expense.recurringTemplate = nil }
            }
        }
    ...
    func deleteRecurringExpenses(_ recurringExpenses: [ReceiptScannerExpenseTracker.RecurringExpense], deleteGeneratedExpenses: Bool = false) {
    ...
    private func hasExpenseForDate(_ date: Date, merchant: String, amount: NSDecimalNumber) -> Bool {
        ...
        let request: NSFetchRequest<ReceiptScannerExpenseTracker.Expense> = ReceiptScannerExpenseTracker.Expense.fetchRequest()
        request.predicate = NSPredicate(
            format: "merchant == %@ AND date >= %@ AND date < %@ AND amount == %@", merchant, startOfDay as CVarArg,
            endOfDay as CVarArg, amount)
        do {
    ...
    func findPotentialDuplicates(for recurringExpense: ReceiptScannerExpenseTracker.RecurringExpense, within days: Int = 3) -> [ReceiptScannerExpenseTracker.Expense] {
    ...
    func detectTemplateLinkedExpenseModification(_ expense: ReceiptScannerExpenseTracker.Expense) -> Bool {
        guard let template = expense.recurringTemplate else { return false }
        // Check if key fields have been modified from the template
        return expense.amount != template.amount || expense.merchant != template.merchant
            || expense.category != template.category || expense.notes != template.notes
            || expense.paymentMethod != template.paymentMethod || expense.currencyCode != template.currencyCode
    }
    /// Get the differences between an expense and its recurring template
    func getExpenseTemplateChanges(_ expense: ReceiptScannerExpenseTracker.Expense) -> [TemplateChangeType] {
        guard let template = expense.recurringTemplate else { return [] }
        var changes: [TemplateChangeType] = []
        if expense.amount != template.amount { changes.append(.amount(from: template.amount, to: expense.amount)) }
        ...
        if expense.notes != template.notes { changes.append(.notes(from: template.notes, to: expense.notes)) }
        ...
        // Check tags
        let expenseTags = Set(expense.tags?.allObjects as? [ReceiptScannerExpenseTracker.Tag] ?? [])
        let templateTags = Set(template.tags?.allObjects as? [ReceiptScannerExpenseTracker.Tag] ?? [])
        if expenseTags != templateTags { changes.append(.tags(from: Array(templateTags), to: Array(expenseTags))) }
        return changes
    }
    /// Update a recurring template from expense changes
    func updateTemplateFromExpense(_ template: ReceiptScannerExpenseTracker.RecurringExpense, with changes: [TemplateChangeType]) throws {
        guard !template.isDeleted, template.managedObjectContext != nil else {
            throw RecurringExpenseError.templateNotFound
        }
        for change in changes {
            switch change {
            case .amount(_, let newAmount): template.amount = newAmount
            case .merchant(_, let newMerchant): template.merchant = newMerchant
            case .category(_, let newCategory): template.category = newCategory
            case .notes(_, let newNotes): template.notes = newNotes
            case .paymentMethod(_, let newPaymentMethod): template.paymentMethod = newPaymentMethod
            case .currency(_, let newCurrency): template.currencyCode = newCurrency
            case .tags(_, let newTags):
                // Clear existing tags
                if let existingTags = template.tags { template.removeFromTags(existingTags) }
                // Add new tags
                for tag in newTags { template.addToTags(tag) }
            }
        }
    }
    /// Synchronize template from a specific expense
    func synchronizeTemplateFromExpense(_ expense: ReceiptScannerExpenseTracker.Expense) throws {
        guard let template = expense.recurringTemplate else { throw RecurringExpenseError.templateNotFound }
    ...
    func batchSynchronizeTemplateFromExpenses(_ expenses: [ReceiptScannerExpenseTracker.Expense]) throws {
        // Group expenses by their recurring template
        var expensesByTemplate: [ReceiptScannerExpenseTracker.RecurringExpense: [ReceiptScannerExpenseTracker.Expense]] = [:]
        for expense in expenses {
            guard let template = expense.recurringTemplate else { continue }
            if expensesByTemplate[template] == nil { expensesByTemplate[template] = [] }
    ...
    private func synchronizeTemplateFromMultipleExpenses(_ template: ReceiptScannerExpenseTracker.RecurringExpense, expenses: [ReceiptScannerExpenseTracker.Expense]) throws {
        ...
        for expense in expenses {
            let changes = getExpenseTemplateChanges(expense)
            if !changes.isEmpty { allChanges.append(changes) }
        }
    ...
    func rollbackTemplateSynchronization(_ template: ReceiptScannerExpenseTracker.RecurringExpense, originalValues: [String: Any]) throws {
        guard !template.isDeleted, template.managedObjectContext != nil else {
            throw RecurringExpenseError.templateNotFound
        }
        // Restore original values
        if let amount = originalValues["amount"] as? NSDecimalNumber { template.amount = amount }
        if let merchant = originalValues["merchant"] as? String { template.merchant = merchant }
        if let category = originalValues["category"] as? ReceiptScannerExpenseTracker.Category { template.category = category }
        if let notes = originalValues["notes"] as? String? { template.notes = notes }
        if let paymentMethod = originalValues["paymentMethod"] as? String? { template.paymentMethod = paymentMethod }
        if let currencyCode = originalValues["currencyCode"] as? String { template.currencyCode = currencyCode }
        if let tags = originalValues["tags"] as? [ReceiptScannerExpenseTracker.Tag] {
            // Clear existing tags
            if let existingTags = template.tags { template.removeFromTags(existingTags) }
            // Restore original tags
            for tag in tags { template.addToTags(tag) }
        }
    }
    /// Create a snapshot of template values for rollback purposes
    func createTemplateSnapshot(_ template: ReceiptScannerExpenseTracker.RecurringExpense) -> [String: Any] {
        ...
        snapshot["tags"] = Array(template.tags?.allObjects as? [ReceiptScannerExpenseTracker.Tag] ?? [])
        return snapshot
    }
    /// Validate template synchronization integrity
    func validateTemplateSynchronization(_ template: ReceiptScannerExpenseTracker.RecurringExpense, changes: [TemplateChangeType]) -> Bool {
        // Check that the template is still valid after changes
        guard !template.isDeleted, template.managedObjectContext != nil, !template.merchant.isEmpty,
            template.amount.doubleValue > 0
        else { return false }
        // Validate that changes are reasonable
        for change in changes {
            switch change {
            case .amount(_, let newAmount): if newAmount.doubleValue <= 0 { return false }
            case .merchant(_, let newMerchant): if newMerchant.isEmpty { return false }
            case .currency(_, let newCurrency): if newCurrency.isEmpty { return false }
            default: break
            }
        }
        return true
    ...
    func validateTemplateNotOrphaned(_ template: ReceiptScannerExpenseTracker.RecurringExpense) -> Bool {
        ...
        if !generatedExpenses.isEmpty { return true }
        // Check if it's been inactive for more than 6 months
        let sixMonthsAgo = Calendar.current.date(byAdding: .month, value: -6, to: Date()) ?? Date()
        if template.createdDate < sixMonthsAgo && !template.isActive { return false }
        // Check if next due date is more than 3 months in the past
        if let pattern = template.pattern {
            let threeMonthsAgo = Calendar.current.date(byAdding: .month, value: -3, to: Date()) ?? Date()
            if pattern.nextDueDate < threeMonthsAgo && template.lastGeneratedDate == nil { return false }
        }
        return true
    }
    /// Find and clean up orphaned templates
    func findOrphanedTemplates() -> [ReceiptScannerExpenseTracker.RecurringExpense] {
    ...
    func resolveTemplateUpdateConflicts(_ template: ReceiptScannerExpenseTracker.RecurringExpense, conflictingChanges: [[TemplateChangeType]]) throws
        -> [TemplateChangeType]
    {
        ...
        for changeSet in conflictingChanges {
            for change in changeSet {
                ...
                if changesByType[key] == nil { changesByType[key] = [] }
    ...
    private func resolveAmountConflict(_ changes: [TemplateChangeType]) -> TemplateChangeType {
        ...
        for change in changes {
            if case .amount(_, let newAmount) = change { amountCounts[newAmount] = (amountCounts[newAmount] ?? 0) + 1 }
        }
    ...
    private func resolveCategoryConflict(_ changes: [TemplateChangeType]) -> TemplateChangeType {
        var categoryCounts: [ReceiptScannerExpenseTracker.Category?: Int] = [:]
        for change in changes {
    ...
    private func resolveNotesConflict(_ changes: [TemplateChangeType]) -> TemplateChangeType {
        var allNotes: [String] = []
        for change in changes {
            if case .notes(_, let newNotes) = change, let notes = newNotes, !notes.isEmpty { allNotes.append(notes) }
        }
    ...
    private func resolveTagsConflict(_ changes: [TemplateChangeType]) -> TemplateChangeType {
        var allTags: Set<ReceiptScannerExpenseTracker.Tag> = []
        for change in changes { if case .tags(_, let newTags) = change { allTags.formUnion(newTags) } }
        return .tags(from: [], to: Array(allTags))
    }
    // MARK: - Helper Methods

    private func calculateInitialDueDate(
        from startDate: Date, pattern: RecurringFrequency, interval: Int32, dayOfMonth: Int32?
    ) -> Date {
        let calendar = Calendar.current
        switch pattern {
        case .none: return startDate
        case .weekly: return calendar.date(byAdding: .weekOfYear, value: Int(interval), to: startDate) ?? startDate
        case .biweekly:
        ...
        case .quarterly: return calendar.date(byAdding: .month, value: 3 * Int(interval), to: startDate) ?? startDate
        }
    }
