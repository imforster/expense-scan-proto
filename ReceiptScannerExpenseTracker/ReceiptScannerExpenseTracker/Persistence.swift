...
class PersistenceController: ObservableObject {
    ...
    @Published var isStoreLoaded = true // Always true since we're using CoreDataManager

    // Preview helper for SwiftUI previews - now uses CoreDataManager
    @MainActor
    ...
    init(inMemory: Bool = false) {
        if inMemory {
            // For previews, create a separate in-memory container
            ...
            // For production, delegate to CoreDataManager
